...
Main Hoop until it meets the two of:   ( refer to F.6.4 and F.7.5.1 )
...
feet and legs ahead of the Front Hoop must not extend above or outside Structural Tubing or
Equivalent
...
## T.1.3.4 Covers over suspension and steering components must be removable to inspect the mounting
points
...
make sure that the driver will not contact any metal or other materials which may become
heated to a surface temperature above 60°C.
...
include but are not limited to:  exhaust pipe, coolant hose/tube, Tractive Battery Pack) and a
place that the driver could contact (including seat or floor):
...
## T.1.8 Firewall
## T.1.8.1 A Firewall(s) must separate the driver compartment and any portion of the Driver Harness
from:
...
and cable to those Motors where mounted at the wheels or on the front control arms
## T.1.8.2 The Firewall must extend sufficiently far upwards and/or rearwards and/or sideways where
...
## T.1.8.6 Sealing
a. Any Firewall must seal completely against the passage of fluids (the Firewall itself, joints,
edges, any pass throughs and Floor Closeout)
...
permitted.  “Y” type shoulder straps are not permitted.  The “H” type configuration is
permitted.
...
c. Not cause abrasion to the belt webbing
Bolt tabs are checked in double tearout
...
the chassis
b. Welded tabs or eyes must have a minimum base dimension of the outer diameter of the
tab or eye
c. Where a single shear tab is welded to the chassis, the tab to tube welding must be on the
two sides of the base of the tab
Double shear attachments are preferred.  Tabs and brackets for double shear mounts should
be welded on the two sides.
## T.2.4.5 Eyebolts or Weld Eyes must:
a. Be one piece. No eyenuts or swivels.
b. Be harness manufacturer supplied
Threads should be 7/16-20 or greater
c. Be aligned to let the harness clip-on bracket pivot freely, and not touch other harness
brackets (lap and anti sub) or other vehicle parts
d. Have a positive locking feature on threads or by the belt itself
e. Weld Eyes must have a shank inserted through a Welded Insert F.3.4.3
...
c. Be properly aligned with the clip-on harness bracket (not twisted) to prevent the belt
creating a torque on the eyebolt.
...
a. Bolt or eyebolt through a welded tube insert or tested monocoque attachment  F.7.8.8
b. Bolt to a tab or bracket, or clip to an eye ( T.2.4.3 ) on a tube frame
...
b. Bolt through a welded tube insert or tested monocoque attachment  F.7.8.8
c. Bolt to a well gusseted tab behind the Shoulder Harness Mounting Bar or clip to an eye (
## T.2.4.3 ) loaded in tension on the Shoulder Harness Mounting bar
...
View Angle up to 20° rearwards.
The Anti-Submarine Belt Mount Spacing should be approximately 100 mm apart.
...
## T.2.8.5 The Head Restraint, attachment and mounting must be strong enough to withstand a
minimum force of:
...
b. The remaining Brake Pedal travel must directly operate the hydraulic brake system.
...
normal range will operate the switch
## T.3.3.2 The BOTS must:
a. Be a mechanical single pole, single throw (two position) switch (push-pull, push-rotate or
flip type)
...
## T.3.3.5 The BOTS circuit must be implemented with analog components, not using programmable
logic controllers, engine control units, or similar functioning digital controllers
## T.3.4 Brake Light
...
• Have different transfer functions which have different gradients and/or offsets to the
other(s)
• Have a pull-up or pull-down resistor in circuit to bring an open circuit input to 0% pedal
travel
• At any pedal position above 10%, the pedal travel output of a single sensor must differ by
minimum 10% from any other sensor
Opposite slope sensors that do not meet this requirement will not be permitted
...
## T.4.2.6 If three sensors are used, then if one APPS failure, any two sensors that agree within 10%
Pedal Travel may be used to define the (IC) throttle position / (EV) torque target and the 3rd
APPS may be ignored.
...
The circuitry used to evaluate the sensor must use pull down or pull up resistors to make sure
...
a. (IC Only) The ETC Systems Form must contain a detailed description of all the potential
failure modes that can occur, the strategy that is used to detect these failures and the
tests that have been conducted to prove that the detection strategy works
...
• An inline switchable breakout box available that may disconnect each BSE signal(s) to the
main ECU without affecting any other connections
...
electric motors, must be fitted with scatter shields to contain drivetrain parts during radial
failure
...
a. Be made of 2.66 mm (0.105 inch) minimum thickness steel (no alternatives are
permitted)
...
• Minimum thickness for aluminum alloy:  3.0 mm
...
suspension components are permitted.
...
## T.8.2.4 All Critical Fasteners must be secured from unintentional loosening with Positive Locking
Mechanisms see T.8.3
## T.8.2.5 A minimum of two full threads must project from any lock nut
## T.8.2.6 Some Critical Fastener applications have additional requirements that are provided in the
applicable section
...
meet the positive locking requirement.  A latch requires an additional Positive Locking
Mechanism
...
b. A supply of replacement fasteners must be presented in Technical Inspection, including
...
## T.9.2.1 All Low Voltage Batteries and onboard power supplies must be fully and securely mounted
inside chassis structure below the height of the Shoulder Belt Mount T.2.6
This chassis structure must be sufficient for suspension mounting
...
8 g and 11 g depending on the duration of the deceleration
